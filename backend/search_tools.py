...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        for doc, meta in zip(results.documents, results.metadata, strict=False):
            ...
            # Retrieve URL from vector store
            source_url = None
            if lesson_num is not None:
                # Get lesson-specific link
                source_url = self.store.get_lesson_link(course_title, lesson_num)
            else:
                # Get course-level link
...
class CourseOutlineTool(Tool):
    ...
    def get_tool_definition(self) -> dict[str, Any]:
        ...
        return {
            ...
            "description": "Get complete course outline including title, link, and list of all lessons with their numbers and titles",
            "input_schema": {
                "type": "object",
                "properties": {
                    "course_title": {
    ...
    def execute(self, course_title: str) -> str:
        ...
        Args:
            course_title: Course title (fuzzy matching supported)
        ...
        try:
            ...
            if not results or not results.get("metadatas") or not results["metadatas"]:
                return f"No course data found for '{resolved_title}'"
            ...
            # Format output
            output_lines = [f"Course: {title}", f"Link: {course_link}", "", "Lessons:"]
